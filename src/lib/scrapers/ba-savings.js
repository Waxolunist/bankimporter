    ...
    steps: [new DL.Step('Start',
        ...
        */
        ...
        new DL.Step('Waitfordownloadlink',
            async function(browser, page, args) {
                await args.frame.waitForSelector('#generalForm\\:j_id29\\:0\\:j_id35\\:exporterExcel', { visible: true });
            }),
        new DL.Step('Clickdownload',
            async function(browser, page, args) {
                args.frame.$eval('#generalForm\\:j_id29\\:0\\:j_id35\\:exporterExcel\\:j_id36\\:j_id41\\:link', a => a.click());
