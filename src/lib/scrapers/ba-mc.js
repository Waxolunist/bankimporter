    ...
    steps: [new DL.Step('Start',
        ...
        new DL.Step('Banking',
            async function(browser, page, args) {
                ...
                if (page.url().startsWith("https://banking.bankaustria.at")) {
                    console.log("Forward to old banking.");
                    await page.goto("https://banking.bankaustria.at/EPP-ESA-WS/GoToExternalSite?destination=oldVersionSite");
                    console.log("Old Banking started");
                }
        ...
        */
        ...
        new DL.Step('Waitfordownloadlink',
            async function(browser, page, args) {
                await args.frame.waitForSelector('#generalForm\\:detailPage\\:j_id27\\:0\\:j_id36\\:exporterExcel', { visible: true });
            }),
        new DL.Step('Clickdownload',
            async function(browser, page, args) {
                args.frame.$eval('#generalForm\\:detailPage\\:j_id27\\:0\\:j_id36\\:exporterExcel\\:j_id37\\:j_id42\\:link', a => a.click());
